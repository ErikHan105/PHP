...
if($searchby == 'departement')
{
	...
	$_req = $r->fetch();
	$depname = $_req['departement_nom'];
	$dep = '<a href="'.$url_script.'/offre.php?departement='.$departement.'">'.$depname.'</a> >';
}
else if($searchby == 'region')
{
	...
	$dep = '<a href="'.$url_script.'/offre.php?region='.$_req['idmap'].'">'.$_req['titre'].'</a> >';
}
else
{
	$dep = '';
...
if($champs_pays != '')
{
	$champs_pays = '<a href="'.$url_script.'/offre.php">'.$champs_pays.'</a> > ';
}
...
$navigation = '<a href="'.$url_script.'">Accueil</a> > '.$champs_pays.$dep.' <a href="'.$url_script.'/offre.php?categorie='.$idcategorie.'">'.$namecategorie.'</a> > <a href="'.$url_annonce.'">'.$titre.'</a>';
...
if($pro == 'yes')
{
	$class_templateloader->assign("{utilisateur}",'<div class="pro-title"><a href="'.$url_script.'/boutique.php?md5='.$md5.'">'.$username.'</a></div>');
...
$class_templateloader->assign("{codepostal}",$codepostal);
...
$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".$ville."&key=AIzaSyCjd0AaXmIV3o665Jwy7wKlRw1U_SF9_dU&callback=initMap";
...
$class_templateloader->assign("{map}",$geoplugin->countryName);
$class_templateloader->assign("{lat}",$lat);
$class_templateloader->assign("{lng}",$lng);
...
$class_templateloader->assign("{contact_user}",$url_script."/contact_user.php?md5=".$md5);
