...
if(isset($_REQUEST['action']))
{
	...
	if($action == 3)
	{
		...
		$message .= 'Votre annonce "'.$titreannonce.'" vient d\'être valider et est désormais visible sur '.$url_script.'<br><br>';
		$message .= 'Cordialement,<br>';
		$message .= $url_script;
		$class_email->sendMailTemplate($email,'Votre annonce "'.$titreannonce.'" vient d\'être valider sur '.$url_script,$message);
			...
			<form>
				...
				while($req = $reponse->fetch())
				{
					...
					else
					{
						...
						{
						?>
							<option value="<?php echo $req['id']; ?>" selected><?php echo $req['titre']; ?></option>
						<?php
						}
						else
						{
