...
if($listing_type_recherche == 'ville')
{
	...
	while($req = $reponse->fetch())
	{
		...
		$nom = str_replace("'","\'",$nom);
		?>
		<li><a href="javascript:void(0);" onclick="updateText('<?php echo $req['codepostal']; ?>','<?php echo $nom; ?>','<?php echo $req['idregion']; ?>');"><?php echo $req['nom']; ?></a></li>
		<?php
	}
	?>
	</ul>
...
else if($listing_type_recherche == 'codepostal')
{
	...
	while($req = $reponse->fetch())
	{
		...
		$nom = str_replace("'","\'",$nom);
		?>
		<li><a href="javascript:void(0);" onclick="updateText('<?php echo $req['codepostal']; ?>','<?php echo $nom; ?>','<?php echo $req['idregion']; ?>');"><?php echo $req['codepostal']; ?></a></li>
		<?php
	}
	?>
	</ul>
