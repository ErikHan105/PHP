...
class Search
{
	...
	function setSearchBoutiqueCount($page,$searchtext,$categorie,$region,$departement,$tri,$codepostal)
	{
		...
		$SQL = "SELECT COUNT(*) FROM pas_compte_pro INNER JOIN pas_user ON pas_compte_pro.md5 = pas_user.md5 $addcategorie $addsearchtext";
		$reponse = $pdo->query($SQL);
		$req = $reponse->fetch();
	...
	function setSearchBoutique($page,$searchtext,$categorie,$region,$departement,$tri,$codepostal)
	{
		...
		$SQL = "SELECT * FROM pas_compte_pro INNER JOIN pas_user ON pas_compte_pro.md5 = pas_user.md5 $addcategorie $addsearchtext";
		$reponse = $pdo->query($SQL);
		while($req = $reponse->fetch())
		{
			$SQL = "SELECT COUNT(*) FROM pas_annonce WHERE iduser = ".$req['id'];
			$r = $pdo->query($SQL);
			$rr = $r->fetch();
	...
	function setSearchCount($page,$searchtext,$categorie,$type,$region,$departement,$tri,$codepostal)
	{
		...
		$SQLCount = "SELECT COUNT(*) FROM pas_annonce WHERE valider = 'yes' AND offre_demande = '$type' $addcategorie $addsearchtext $addregion $adddepartement $addcodepostal $tri";
	...
	function setSearch($page,$searchtext,$categorie,$type,$region,$departement,$tri,$codepostal)
	{
		...
		$SQL = "SELECT * FROM pas_annonce WHERE valider = 'yes' AND offre_demande = '$type' $addcategorie $addsearchtext $addregion $adddepartement $addcodepostal $tri LIMIT ".($page*$nbrAnnonceParPage).",$nbrAnnonceParPage";
		$reponse = $pdo->query($SQL);
		while($req = $reponse->fetch())
		{
			$md5 = $req['md5'];
