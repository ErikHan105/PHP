	...
	if(date('d') == $date_remonter[2])
	{
		...
		if(date('m') == $date_remonter[1])
		{
			...
			if(date('Y') == $date_remonter[0])
			{
				...
				$SQL = "INSERT INTO pas_annonce (iduser,slug,idregion,md5,titre,texte,offre_demande,idcategorie,prix,codepostal,valider,date_soumission,nbr_vue,telephone,ville,urgente,quinzejour,unmois,status) VALUES ('$iduser','$slug',$idregion,'$md5','$titre','$texte','$offre_demande',$idcategorie,'$prix','$codepostal','$valider',NOW(),$nbr_vue,'$telephone','$ville',$urgente,'$quinzejour','$unmois','$status')";
...
if($duree != 0)
{
	...
	while($req = $reponse->fetch())
	{
		...
		if($date['day'] > $duree)
		{
			...
			$messageHTML = str_replace("[link_renew]",'<a href="'.$url_script.'/deposer-une-annonce.php?step=2&md5='.$md5.'">Renouveller votre annonce</a>',$messageHTML);
			...
			$messageTEXTE = str_replace("[link_renew]",$url_script.'/deposer-une-annonce.php?step=2&md5='.$md5,$messageTEXTE);
