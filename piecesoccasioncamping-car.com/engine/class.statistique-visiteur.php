...
class Statistique_Visiteur
{
	...
	function getVisiteur($date)
	{
		...
		/* Si le repertoire cache n'existe pas on le créer */
		if(!is_dir("cache/"))
		{
			mkdir("cache/",0777);
		}
		
		/* Si plus grand il y'a forcement pas de visite */
		if($dayDemande > $day)
		{
			$total = 0;
		}
		...
		{
			/* Si egal on fait toujours la demande */
			if($dayDemande == $day)
			{
			...
			{
				/* On check si un cache existe */
				if(file_exists("cache/".$year."_".$month."_".$dayDemande.".cache"))
				{
					$total = file_get_contents("cache/".$year."_".$month."_".$dayDemande.".cache");
				}
				else
				{
					/* Il existe pas on demande a SQL est on stock */
					$SQL = "SELECT DISTINCT(ip) FROM statistique_visiteur WHERE date_de_la_visite like '$date%' AND `user_agent` NOT LIKE '%bot%'";
					$reponse = $pdo->query($SQL);
					while($req = $reponse->fetch())
					{
					...
					file_put_contents("cache/".$year."_".$month."_".$dayDemande.".cache",$total);
				}
			}			
		}
