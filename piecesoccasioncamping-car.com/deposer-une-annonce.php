...
if(isset($_REQUEST['action']))
{
	...
	if($action == 1)
	{
		...
		$SQL = "INSERT INTO pas_annonce (iduser,slug,idregion,md5,titre,texte,offre_demande,idcategorie,prix,codepostal,valider,date_soumission,nbr_vue,telephone,ville,urgente,quinzejour,unmois,status,pro) VALUES ($iduser,'$slug',$idregion,'$md5annonce','$titre','$texte','$type_annonce',$categorie,'$prix','$codepostal','$moderation_activer',NOW(),0,'$telephone','$ville',1,'no','no','NOVISIBLE','$pro')";
		$pdo->query($SQL);
		...
		if(isset($_REQUEST['step']))
		{
			...
			if($step == 2)
			{
				...
				if($packphoto == 'yes')
				{
					...
					$data .= '<input type="checkbox" id="packphoto" name="packphoto" value="yes" checked disabled> Pack de '.getConfig("nbr_max_photo_payant").' Photos supplémentaire';
					$data .= '</div>';
					$data .= '<div style="float:right;font-size:18px;">';
					$data .= 'Prix : '.getConfig("prix_photo_supplementaire").' '.$currencysigle;
					$data .= '</div>';
					$data .= '</div>';
					$data .= '<p>';
					$data .= 'Cette options ajouter '.getConfig("nbr_max_photo_payant").' photos supplémentaire à votre annonces.';
					$data .= '</p>';*/
   ...
   <style>
				  ...
				  <label>Categorie :</label>
					...
					if($module_filtre_auto == 'true' && $module_filtre_immo == 'true')
					{
						...
						function updateFiltre()
						{
							...
							if(categorie == 113 || categorie == 99 || categorie == 117 || categorie == 90) {
                        	    $(".for_prix").hide();
                        	    $("#prix").val("0");
                        	    $("#telephone").val("");
                        	    $(".for_phonenumber").show();
                        	} else if(categorie == 98 || categorie == 115) {
                        	    $(".for_prix").hide();
                        	    $(".for_phonenumber").hide();
                        	    $("#prix").val("0");
                        	    $("#telephone").val("1111111114");
                        	} else {
                        	    $(".for_prix").show();
                        	    $(".for_phonenumber").show();
                        	    $("#prix").val("");
					...
					?>
					<input type="text" id="prix" name="prix" class="inputbox price"> <b><?php echo $currencysigle; ?></b>
					</div><br>
					<div class="for_phonenumber">
    ...
    $(document).ready(function(){
        ...
        if(categorie == 113 || categorie == 99 || categorie == 117 || categorie == 90) {
    	    ...
    	    console.log("everything have to shows")
    	    $(".for_prix").show();
    	    $(".for_phonenumber").show();
    	    $("#prix").val("");
    	    $("#telephone").val("");
